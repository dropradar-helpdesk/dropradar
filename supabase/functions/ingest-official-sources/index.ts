...
function throwSupabase(label: string, error: unknown) {
...
async function handleRequest(req: Request) {
  ...
  const runInsert = await supabase
    ...
    .select("id")
    .single();
  if (runInsert.error) return json({ error: `create ingest run: ${errorMessage(runInsert.error)}` }, 500);
  ...
  try {
    ...
    throwSupabase("read official_sources", sourceError);
    ...
    throwSupabase("read tracking_request_feed", requestError);
    ...
    for (const source of (sources || []) as OfficialSource[]) {
      ...
      pageAttempts.forEach((attempt, index) => {
        ...
        fetchErrors.push({
          ...
          error: errorMessage(attempt.reason)
        });
      });
      ...
      if (requestMatches.length) {
        const existing = await supabase
          ...
          .limit(1000);
        throwSupabase(`read existing request candidates for ${source.id}`, existing.error);
      ...
      if (!dryRun) {
        ...
        throwSupabase(`insert source_check for ${source.id}`, checkInsert.error);
      }
      ...
      for (const link of generalAddedLinks) {
        ...
        if (!dryRun) {
          ...
          throwSupabase(`insert official diff candidate for ${source.id}`, candidateInsert.error);
        }
      }
      ...
      for (const match of newRequestMatches) {
        ...
        if (!dryRun) {
          ...
          throwSupabase(`insert request candidate for ${source.id}`, candidateInsert.error);
        }
      }
    ...
    return json({ runId, error: message, sourceCount, candidateCount, results }, 500);
  }
}
